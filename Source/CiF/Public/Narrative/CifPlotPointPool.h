...
public:
    ...
    /**
     * Finds the first available plot-point revelation route supported by the named revealer.
     * @param revealer character, item, or other named game object attempting the revelation
     * @param outSelection receives both the plot point and the revelation route that matched
     * @return true when a matching available revelation was found
     * 
     * todo: later this might need to select revelation more cleverly if few are available
     */
    bool findAvailableRevelation(const FName revealer, FCifPlotPointSelection& outSelection) const;
    ...
    /*
        ...
        void plotPointMixIn(const FName initiator, const FName responder, const LineOfDialogue& lod, uint8 lineNumber, piece:String,
